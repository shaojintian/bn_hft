...
def message_handler(_, message):
    with lock:
        ...
        if position == 0 and buy_in_signal:
            ...
            if not success:
                logging.debug("止盈失败重新下: price:%f amount:%f" % (buy_in_price, ORDER_AMOUNT))
                return
...
def handle_signal(signum, frame):
    ...
    # 计算小于0的比例
    negative_ratio = negative_count / len(PRICE_CHANGE_AFTER_SIGNAL_DELAY) * 10
    print("\n%ds后小于0的比例为: %f\n" %(INTERVAL,negative_ratio))
    print("------FINAL PROFIT-------:%f" % profit)
    profit_per_day = (profit / ((end_time - start_time) / 60)) * 24 * 60
    profit_per_month = profit_per_day * 30 * USDCNY
