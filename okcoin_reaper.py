...
class LeeksReaper():
    ...
    def updateOrderBook(self):
        ...
        if (len(orderBook.Bids) < 3 or len(
                orderBook.Asks) < 3):  # 前半段是判断orderBook.Bids的长度是否小于3，后半段是判断orderBook.Asks的长度是否小于3，如果两边都小于3就执行下方的语句
            # 返回undefined
            return
    ...
    def balanceAccount(self):
        ...
        if (len(self.orderBook.Bids) > 0 and now - self.preCalc > (CalcNetInterval)):
            ...
            net = _N(account.Balance + account.FrozenBalance + self.orderBook.Bids[0].Price * (
                        account.Stocks + account.FrozenStocks))
    ...
    def poll(self):
        ...
        if (self.numTick > 2 and (self.prices[-1] - max(self.prices[-6:-1]) > burstPrice or self.prices[-1] - max(
                self.prices[-6:-2]) > burstPrice and self.prices[-1] > self.prices[-2])):
            # 变量bull赋值为true,tradeAmount赋值为self.cnyv/self.bidPrice乘以0.99
            bull = True
            tradeAmount = self.cny / self.bidPrice * 0.99

        # 同上面if
        elif (self.numTick > 2 and (self.prices[-1] - min(self.prices[-6:-1]) < -burstPrice or self.prices[-1] - min(
                self.prices[-6:-2]) < -burstPrice and self.prices[-1] < self.prices[-2])):
            bear = True
            # 赋值
