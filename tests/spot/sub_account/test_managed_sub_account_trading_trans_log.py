...
def test_managed_sub_account_trading_trans_log_with_missing_parameters(params):
    """Tests the API endpoint to query managed sub account transfer log with missing parameters"""

    client = Client(key, secret)
    client.managed_sub_account_trading_trans_log.when.called_with(
        **params
...
@mock_http_response(
    ...
    + encoded_string(params),
    mock_item,
    200,
)
def test_managed_sub_account_trading_trans_log():
    """Tests the API endpoint to query managed sub account transfer log"""

    client = Client(key, secret)
    response = client.managed_sub_account_trading_trans_log(**params)
